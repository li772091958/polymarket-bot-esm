...
import { splitPosition } from '../polymarket/relayer.js';

type SplitMode = 'slug' | 'condition';
...
function usage(message?: string) {
  return [
    'Usage:',
    '  npm run split -s <slug> <amount>',
    '  npm run split -c <conditionId> <amount>',
    '  npm run split -- -s <slug> <amount>',
    '  npm run split -- -c <conditionId> <amount>',
    message ? `\n${message}` : '',
  ].join('\n');
...
function inferMode(identifier: string): SplitMode {
  return CONDITION_ID_PATTERN.test(identifier) ? 'condition' : 'slug';
}

function parseArgs(args: string[]): ParsedArgs {
  let mode: SplitMode | undefined;
...
async function resolveSplitTarget({ mode, identifier }: ParsedArgs) {
  if (mode === 'slug') {
...
async function main() {
  ...
  try {
    const target = await resolveSplitTarget(parsed);
    ...
    console.error('Failed to split market:', formatError(error));
    exitCode = 1;
  } finally {
